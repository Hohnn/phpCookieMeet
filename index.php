...
$password = $_POST['password'] ?? 'Vide' ;
$confirm = $_POST['confirm'] ?? 'Vide' ;
$regexName = "/^[a-z ,.'-]+$/i";
$regexMail = "/[^@ \t\r\n]+@[^@ \t\r\n]+\.[^@ \t\r\n]+/";
...
<body>
    ...
    <div class="myForm">
        <form action="developpers.php" method="post" novalidate>
        <div class="mb-3">
            <label for="name" class="form-label">Nom</label>
            <input type="text" class="form-control <?= $className ?? '' ?>" id="name" name="name" required value="<?= $name ?? '';?>" > <!-- si il ya le name dans POSt affiche le sinon met rien -->
            <div id="emailHelp" class="form-text"><?= $errorName ?? '' ?></div> <!-- affiche le message d'erreur -->
        </div>
        <div class="mb-3">
            <label for="firstname" class="form-label">Prénom</label>
            <input type="text" class="form-control <?= $classFirstname ?? '' ?>" id="firstname" name="firstname" required value="<?= $firstname ?? '';?>">
            <div id="emailHelp" class="form-text"><?= $errorFirstname ?? '' ?></div>
        </div>
        <div class="mb-3">
            <label for="email" class="form-label">Adresse mail</label>
            <input type="email" class="form-control <?= $classMail ?? '' ?>" id="email" name="email" aria-describedby="emailHelp" required value="<?= $email ?? '';?>">
            <div id="emailHelp" class="form-text"><?= $errorMail ?? '' ?></div>
        </div>
        <div class="mb-3">
            <label for="password" class="form-label">Mot de passe</label>
            <input type="password" class="form-control <?= $classMdp ?? '' ?>" id="password" name="password" required>
            <div id="emailHelp" class="form-text"><?= $errorMdp ?? '' ?></div>
        </div>
        <div class="mb-3">
            <label for="confirm" class="form-label">Confirmation du mot de passe</label>
            <input type="password" class="form-control <?= $classPassword ?? '' ?>" id="password2" name="confirm" required>
